...
Version 1.4.1 reintroduced the noncompactly supported distance function as default behaviour. This results in higher quality maps in general. There is no command line option or a parameter in the interfaces to control this behaviour and change it to a compactly supported distance function. If the change is necessary, line 53 in `mapDistanceFunctions.cpp` has to be modified to `if (false) {`.
