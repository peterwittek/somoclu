...
R interface with CUDA support on Windows cannot be properly built. (The CUDA code that requires VC to compile and Rcpp that requires gcc to compile cannot link without error. Though linked together, function call results in wrong/inaccessible values and crashes the R session.)
